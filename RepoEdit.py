...
# Function to print messages to the workflow logs
def log(message):
    print(message)
    sys.stdout.flush()
...
for repo in g.get_user().get_repos(type="owner"):
    log(f"Processing repository: {repo.name}")
    
    # Check if the repository should be excluded
    if repo.name in EXCLUDED_REPOS:
        log(f"Skipping repository: {repo.name} as it's excluded.")
        continue
    ...
    try:
        ...
        log(f"No Deploy.yml found in the repository: {e}")
    ...
    if NAMESPACE_TO_MATCH != ['']:  # Check if NAMESPACE_TO_MATCH is not an empty string
        if deploy_yml_file:
            ...
            if argo_app not in NAMESPACE_TO_MATCH:
                log(f"Namespace '{argo_app}' does not match, moving to the next repository.")
                continue  # Move to the next repository
    else:
        log("No namespaces provided, skipping namespace matching.")
    ...
    if REPOS_TO_CHANGE != ['']:  # Check if REPOS_TO_CHANGE is not an empty string
        if repo.name not in REPOS_TO_CHANGE:
            log(f"Skipping repository: {repo.name} as it's not in the specified repositories list.")
            continue
    else:
        log("No specific repositories provided, processing all repositories.")
    
    # Iterate through each file in the repository
    for file in repo_contents:
        log(f"Processing file: {file.name}")
        
        # Skip processing excluded files
        if file.name in FILE_EXCLUSIONS:
            log(f"Skipping {file.name} as it's in the exclusions list.")
            continue
        ...
        try:
            ...
            if STR_TO_REPLACE in file_content:
                ...
                log(f"Replaced in {file.name}")

                # Change repository name if specified
                if CHANGE_REPO_NAME:
                    log(f"CHANGE_REPO_NAME: {CHANGE_REPO_NAME}")
                    log(f"STR_TO_REPLACE: {STR_TO_REPLACE}")
                    log(f"Repo name: {repo.name}")
                    
                    if STR_TO_REPLACE in repo.name:
                        log("Attempting to change repository name.")
                        new_repo_name = repo.name.replace(STR_TO_REPLACE, REPLACEMENT_STRING)
                        try:
                            repo.edit(name=new_repo_name)
                            log(f"Repository name changed to: {new_repo_name}")
                        except Exception as e:
                            log(f"Error occurred while changing repository name: {e}")
                    else:
                        log("Not attempting to change repository name")
                else:
                    log("CHANGE_REPO_NAME is False, skipping repository name changes.")
            else:
                log(f"The string {STR_TO_REPLACE} is not found in {file.name}")
        except Exception as e:
            log(f"An error occurred while processing {file.name}: {e}")
